...
public class BattleshipHumanPlayer extends ActionBarActivity implements View.OnClickListener{
    ...
    //hit and miss sound variables/background music
    ...
    private int[][] AIgrid = new int[10][10];//to store AI's grid

    //variables to keep track of life of each AI's ship
        ...
        if (gameState.getPlayerID() == 1) {//if it's AI's turn
            if (AIchoice == 1) {//normal AI
                ...
                while (gameState.getPlayerID() == 1) {//while it is still AI's turn (AI might have chosen a coordinate it previously chose)
                    ...
                    gameState.shipHit(row, col);//fire on this coordinate
                }
            }
    ...
    public void checkHitOrMiss(View view) {
        switch (view.getId()) {//based on button pressed
            case R.id.A1:
                gameState.shipHit(0, 0);//see if ship was hit at coodrdinate A1
            ...
            case R.id.A2:
                gameState.shipHit(0, 1);//see if ship was hit at coodrdinate A2 and so on
    ...
    public void checkIfGameOver (){
        if (gameState.getPlayer1Hits() == 17) {//if user won
            Intent intent = new Intent(this,GameOverPopup.class);//display popup screen
    ...
    /**
     * Description: Method to check if one of the AI ships were sunk. Displays a message on the message screen if so.
     *
     * CAVEAT:
     *
     * @param state
    ...
    {
        AIgrid = state.getComputerGrid();//get AI grid to examine hits

        //0 - ship is facing north
        //1 - ship is facing east
        //2 - ship is facing south
        //3 - ship is facing west

        //if a grid position equals 1, this means there is a hit
